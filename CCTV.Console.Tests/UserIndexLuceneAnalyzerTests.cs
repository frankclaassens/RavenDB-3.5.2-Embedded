...
{
    [TestFixture]
    public class UserIndexLuceneAnalyzerTests
    {
        ...
        public void Confirm_All_Internal_And_External_Users_Have_Been_Indexed()
        {
            ...
            using (var session = _store.OpenSession())
            {
                ...
                var users = session.Query<UserIndexLuceneAnalyzer.Definition, UserIndexLuceneAnalyzer>()
        ...
        private IList<string> LoadInternalUserByIndex(string queryString)
        {
            ...
            using (var session = _store.OpenSession())
            {
                var internalUsers = session.QueryUserIndexLuceneAnalyzer()
                    .QueryMultipleWordsToLowerVarient(x => x.Query, queryString)
                ...
                var externalUsers =
                    session.QueryUserIndexLuceneAnalyzer()
                        .QueryMultipleWordsToLowerVarient(x => x.Query, queryString)
        ...
        {
            // First delete UserIndexLuceneAnalyzer, then create it again.
            try
            {
                _store.DatabaseCommands.DeleteIndex("UserIndexLuceneAnalyzer");
            }
            catch (Exception ex)
            {
                throw new Exception("Cannot delete UserIndexLuceneAnalyzer", ex);
            }

            //IndexCreation.CreateIndexes(typeof(UserIndexLuceneAnalyzer).Assembly, _store);
            var userIndexLuceneAnalyzer = new UserIndexLuceneAnalyzer();
            userIndexLuceneAnalyzer.Execute(_store);
        }
